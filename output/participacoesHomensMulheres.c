...
int main() {
    ...
    for(int i = 0; buscaNOC[i]; i++) buscaNOC[i] = toupper(buscaNOC[i]); // Loop feito para percorrer as palvras atrás de uma letra minúscula, que é transformada em maiúscula ( toupper ) 
    ...
    while (fgets(linha, sizeof(linha), arquivo)) { // percorrerá o arquivo linha por linha até o final dele 
        ...
        if (strstr(linha, buscaNOC) != NULL) { // verifica se a linha que esta sendo percorrida contem o NOC, que o usuário digitou mais cedo por meio do String in String             
            ...
            if (strstr(linha, "Women") != NULL) { // Caso seja medalhista, cria uma condição para quem for mulher, que quando achada acrescentará uma unidade ao contador 
                ...
                m++; // contador de mulher aumenta uma unidade quando achada Women na linha
                ...
                h++; // contador de homem aumenta uma unidade quando achado Men na linha
            }
        }
    } // Fim do while

    printf("Resultados para participações de homens e mulheres de %s\n", buscaNOC); // imprime a mensagem de onde vem os atletas, que o usuário digitiou mais cedo
    printf("Homens participantes: %d\n", h); // mostra o total de homens medalhistas
    printf("Mulheres participantes: %d\n", m); // mostra o toal de mulheres medalhistas
